...
(passLPlot <- passRate %>% 
  ...
  geom_point() + 
  geom_errorbar(aes(ymin = passRate - semPass, ymax = passRate + semPass), width =0.1) + 
  scale_colour_grey(start = .4, end = .8) + theme_minimal() + 
...
(histPassPlot <-passRate %>% 
  ...
  ggplot(aes(x = yrSem, y = passRate, colour = factor(oer), group = factor(oer))) + geom_point() + geom_line()  + theme_minimal() + scale_colour_grey(start = .4, end = .8, labels = c("Traditional Textbook", "Open Textbook")) +
...
(avgGradeLPlot <- avgGrade %>% 
  ...
  scale_colour_grey(start =.4, end = .8) +  scale_fill_grey(start = .4, end = .8) +
...
(histAvgGrade <- avgGrade %>% 
  filter(courseSubject == "HIST" & yrSem %in% pilotSems) %>% 
  ggplot(aes(x = yrSem, y = mGrade, colour = factor(oer), group = factor(oer))) + geom_point() + geom_line(lwd = 1)  + theme_minimal() + scale_colour_grey(start = .4, end = .8, labels = c("Traditional Textbook", "Open Textbook")) + 
...
polsPassOer <- passRate %>% 
  ...
  ggplot(aes(x = yrSem, y = meanPassRate, colour = factor(oer), group = factor(oer))) + geom_point() + geom_line()  + theme_minimal() + 
  scale_colour_grey(start = .4, end = .8, labels = c("Traditional Textbook", "Open Textbook")) + 
  labs(x = "Semester Year", y = "Pass Rate", colour = "Textbook Type") + 
  geom_errorbar(aes(ymin = meanPassRate - meanSemPass, ymax = meanPassRate + meanSemPass, colour = factor(oer)), width = .1) + scale_y_continuous(limits = c(0,1)) + geom_hline(aes(yintercept = .$meanPassRate[.$oer == 1]), lty = 2, lwd = .5, alpha =.2)
...
polsAvgGradeOer <- avgGrade %>% 
  ...
  ggplot(aes(x = yrSem, y = meanGrade, colour = factor(oer), group = factor(oer))) + geom_point() + geom_line()  + theme_minimal() + 
  scale_colour_grey(start = .4, end = .8, labels = c("Traditional Textbook", "Open Textbook")) + 
  labs(x = "Semester Year", y = "Average Grade", colour = "Textbook Type") + 
  geom_errorbar(aes(ymin = meanGrade - meanSem, ymax = meanGrade + meanSem, colour = factor(oer)), width = .1) + scale_y_continuous(limits = c(0,4))+ geom_hline(aes(yintercept = .$meanGrade[.$oer == 1]), lty = 2, lwd = .5, alpha =.2)
