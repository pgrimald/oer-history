...
```{r oerSemSinceImplementation, caption = "Change in the mean percentage of passing students since implementation of the OER.", cache=TRUE}
SLCCpersistenceProc %>% 
...
SLCCpersistenceProc %>% 
  ...
  scale_color_manual(values = osColorPalette[c(4,5,7)]) + labs(x = "Academic Year", y = "Mean Course Grade", colour = "Subject") + scale_fill_manual(values = osColorPalette[c(4,5,7)]) + geom_vline(aes(xintercept = which(levels(yrSem) %in% 'Summer 2014')), lty = 2) + scale_y_continuous(limits = c(0,4))#+ #geom_errorbar(aes(ymin = meanCourseGrade - semCgrade, ymax = meanCourseGrade + semCgrade)) 
...
passMod0Base <- SLCCpersistenceProc %>% 
     glmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId), data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
```

```{r pass Additive Model 1, cache=TRUE}
## Mod1: Random effects + demographics
passMod1Add <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + everPellEligibleInd + firstGenerationInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
```

```{r pass Additive Model 2}
## Mod2: Random effects + Demographics + subject/performance related factors
passMod2Add <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + cumUgGpa + everPellEligibleInd + firstGenerationInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE, optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
...
passMod3Add <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + cumUgGpa + everPellEligibleInd + oer + semSinceImplementation + firstGenerationInd, data = ., family = binomial) #, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE, optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE))
```


```{r pass Interactive model 4, cache=TRUE}
...
passMod4Int <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + gender + firstGenerationInd + fullTime + cumUgGpa + ethnicity + everPellEligibleInd + oer + semSinceImplementation + oer:ethnicity + oer:everPellEligibleInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE, optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
```
...
```{r}
stargazer::stargazer(passMod0, passMod1Add, passMod2Add, passMod4Int, type = "text")
```
