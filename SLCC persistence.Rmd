...
output: 
  html_notebook:
    code_folding: hide
    css: ~/Documents/git/research/eGrader/style.css
    highlight: pygments
    theme: cerulean
    toc: yes
    toc_float:
...
SLCCpersistenceProc %>% 
  ...
  ggplot(aes(x = factor(oer), y = meanPerc)) + 
    geom_col(position = "dodge", fill = "grey70") + 
  geom_text(aes(label = groupSize)) + 
  labs(y = "% of Passing Students", x = "OER") + theme_min()
```
...
SLCCpersistenceProc %>% 
  ...
  labs(x = "Semester since implementation of OER", y = "Mean percentage of passing students") + theme_min()
```
...
passMod2Add <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + courseSubject + cumUgGpa + everPellEligibleInd + firstGenerationInd + semester, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
...
passMod3Add <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + courseSubject + cumUgGpa + everPellEligibleInd + semester + oer + semSinceImplementation + firstGenerationInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
```
...
passMod4Int <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + gender + firstGenerationInd + fullTime + courseSubject + cumUgGpa + ethnicity + everPellEligibleInd + semester + oer + semSinceImplementation + oer:ethnicity + oer:everPellEligibleInd, data = ., family = binomial)
```
...
gradeMod1Add <- SLCCpersistenceProc %>% 
  lmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + everPellEligibleInd + firstGenerationInd, data = .)
```

```{r}
## Mod2: Random effects + Demographics + subject/performance related factors
gradeMod2Add <- SLCCpersistenceProc %>% 
  lmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + courseSubject + cumUgGpa + everPellEligibleInd + firstGenerationInd + semester, data = .)
```

```{r}
## Mod3: Random effects + Demographics + subject/performance related factors + treatment
gradeMod3Add <- SLCCpersistenceProc %>% 
  lmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + semester + courseSubject + cumUgGpa + everPellEligibleInd + oer + semSinceImplementation + firstGenerationInd, data = .)
```

```{r Grade Omnibus Model Interactive, cache=TRUE}
## Mod4: Random effects + Demographics + subject/performance related factors + treatment + interaction terms
gradeMod4Int <- SLCCpersistenceProc %>% 
  lmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + gender + firstGenerationInd + fullTime + courseSubject + semester + oer + ethnicity + everPellEligibleInd + semSinceImplementation + oer:ethnicity + oer:everPellEligibleInd + oer:semSinceImplementation, data = .)
```
