...
SLCCpersistenceProc %>% 
  ...
  scale_color_manual(values = osColorPalette[c(4,5,7)]) + labs(x = "Academic Year", y = "Mean Course Grade", colour = "Subject") + scale_fill_manual(values = osColorPalette[c(4,5,7)]) + geom_vline(aes(xintercept = which(levels(yrSem) %in% 'Su 14')), lty = 2) + scale_y_continuous(limits = c(0,4))#+ #geom_errorbar(aes(ymin = meanCourseGrade - semCgrade, ymax = meanCourseGrade + semCgrade)) 
