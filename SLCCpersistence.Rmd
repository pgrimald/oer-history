...
output: 
  html_document: 
    css: supportingScripts/style.css
    toc: yes
font-family: 'Avenir'     
editor_options: 
...
source("~/Documents/oer-history/supportingScripts/SLCCpreProc.R")
```


<!--# Weekly research meeting feedback
#02/01/2018

Change point modelling

Bundle all the minorities together
History OER:Not our, Pell eligible
Mean grade over time, for each subject -->
...
In addition, this dataset included the above parameters for 2 other courses (Political Science, Economics) that exclusively used conventional textbooks. Data from these 2 courses were used to tease apart any temporal fluctuations in student performance that occurred even without the use of OER. 
...
SLCCpersistenceProc %>%
  ...
  ggplot(aes(x = reorder(ethnicity, percRace), y = percRace)) +
    geom_col() + theme_min() + coord_flip() +
    labs(x = "Ethnicity", y = "Percentage of Students") 
```

The SLCC dataset included `r n_distinct(SLCCpersistenceProc$studentId)` students (female =`r gender$n[1]`, male = `r gender$n[2]`, unreported = `r gender$n[3]`). Out of these students, `r pell$yes`% were pell eligible, `r enrollmentStatus$FullTime`% were in full-time programs, and `r firstGen$yes`% identified as first generation students. 

```{r demoUgGPA, include = FALSE}
## Computing a model to see if the demographic variables determine undergraduate GPA
mod00 <- lm(courseGrade ~ age + ethnicity + gender + everPellEligibleInd + firstGenerationInd, data = SLCCpersistenceProc)
```
...
```{r ethnicity based differences in UG GPA, fig.width=15, warning=FALSE, error=FALSE}
SLCCpersistenceProc %>%
  ggplot(aes(x = ethnicity, y = cumUgGpa)) +
  geom_boxplot(alpha = 0.3) + theme_min() +
  labs(x = "Ethnicity", y = "Course Grade")
```

```{r ageCumGPA, fig.width=15, warning=FALSE, error=FALSE}
SLCCpersistenceProc %>% 
  ggplot(aes(x = age, y = cumUgGpa)) + geom_point(alpha = 0.2) +theme_min() + labs(x = "Age", y = "Cumulative undergraduate GPA")
```
...
SLCCpersistenceProc %>% 
  ...
  group_by(oer, pass) %>% 
  summarise(perc = round((sum(!is.na(pass))/nrow(.)) * 100), groupSize = n()) %>% 
  ggplot(aes(x = oer, y = perc, fill = pass)) + 
    geom_col(position = "dodge") + 
    geom_text(aes(label = groupSize), position = position_dodge(width = 1), vjust = -0.5) + scale_fill_manual(values = osColorPalette) +
    labs(y = "% of Passing Students", x = "OER", fill = "Fail (0) /Pass (1)") + theme_min()
```


```{r oerSemSinceImplementation, caption = "Change in the mean percentage of passing students since implementation of the OER.", include = FALSE}
SLCCpersistenceProc %>% 
  add_count(oer, courseSubject,semSinceImplementation) %>% 
  group_by(oer, courseSubject, pass, semSinceImplementation, n) %>% 
  count(pass) %>% 
  mutate(meanPass = round(nn/n * 100)) %>% 
  ggplot(aes(x = semSinceImplementation, y = meanPass, fill = pass)) + 
  geom_col() + facet_wrap(~courseSubject, scales = "free_x") + 
  labs(x = "Semester since implementation of OER", y = "Mean percentage of students", fill = "Pass") + theme_min() +
  scale_fill_manual(values = osColorPalette)
```

```{r fig.width=10, fig.height=6}
SLCCpersistenceProc %>% 
  group_by(courseSubject, yrSem) %>% 
  filter(courseGrade >=0) %>% 
  summarise(meanCourseGrade = mean(courseGrade, na.rm = TRUE), semCgrade = sd(courseGrade, na.rm = TRUE)/sqrt(sum(!is.na(courseGrade)))) %>% 
  arrange(courseSubject, yrSem) %>% 
  ggplot(aes(x = yrSem, y = meanCourseGrade, colour = courseSubject, fill = courseSubject, group = courseSubject)) + geom_line(lwd = 1, alpha = 0.75) + theme_min() +
  scale_color_manual(values = osColorPalette[c(4,5,7)]) + labs(x = "Academic Year", y = "Mean Course Grade", colour = "Subject") + scale_fill_manual(values = osColorPalette[c(4,5,7)]) + geom_vline(aes(xintercept = which(levels(yrSem) %in% 'F 16')), lty = 2) + scale_y_continuous(limits = c(0,4))#+ #geom_errorbar(aes(ymin = meanCourseGrade - semCgrade, ymax = meanCourseGrade + semCgrade)) 
  #geom_smooth(stat = "smooth", aes(fill = courseSubject))
...
SLCCpersistenceProc %>% 
  group_by(courseSubject, yrSem) %>% 
  #filter(dfw == 1) %>% View()
  summarise(dfwRate = sum(dfw == 1)/sum(!is.na(dfw)), ndfwRate = sum(dfw == 0)/sum(!is.na(dfw))) %>% 
  ggplot(aes(x = yrSem, y = dfwRate, colour = courseSubject, fill = courseSubject, group = courseSubject)) + geom_line(lwd = 1, alpha = 0.75) + theme_min() +
  scale_color_manual(values = osColorPalette[c(4,5,7)]) + labs(x = "Semester-Year", y = "DFW Rate", colour = "Subject") + scale_fill_manual(values = osColorPalette[c(4,5,7)]) + geom_vline(aes(xintercept = which(levels(yrSem) %in% c('Sp 16'))), lty = 2) + scale_y_continuous(limits = c(0,1))#+ #geom_errorbar(aes(ymin = meanCourseGrade - semCgrade, ymax = meanCourseGrade + semCgrade)) 
  #geom_smooth(stat = "smooth", aes(fill = courseSubject))
# + scale_x_date(labels = date_format("%m-%Y"), date_breaks = "8 months") 
```
...
SLCCpersistenceProc %>% 
  ...
  #filter(dfw == 1) %>% View()
  summarise(dfwRate = sum(dfw == 1)/sum(!is.na(dfw)), ndfwRate = sum(dfw == 0)/sum(!is.na(dfw))) %>% 
  ggplot(aes(x = yrSem, y = ndfwRate, colour = courseSubject, fill = courseSubject, group = courseSubject)) + geom_line(lwd = 1, alpha = 0.75) + theme_min() +
  scale_color_manual(values = osColorPalette[c(4,5,7)]) + labs(x = "Semester-Year", y = "Pass Rate", colour = "Subject") + scale_fill_manual(values = osColorPalette[c(4,5,7)]) + geom_vline(aes(xintercept = which(levels(yrSem) %in% 'F 16')), lty = 2) + scale_y_continuous(limits = c(0,1))#+ #geom_errorbar(aes(ymin = meanCourseGrade - semCgrade, ymax = meanCourseGrade + semCgrade)) 
  #geom_smooth(stat = "smooth", aes(fill = courseSubject))
# + scale_x_date(labels = date_format("%m-%Y"), date_breaks = "8 months") 
```

```{r prepostOERgrade, caption = "Change in mean grade of students before and after introduction of OER.", cache=TRUE}
SLCCpersistenceProc %>% 
  ...
  ggplot(aes(x = factor(oer), y = meanGrade, fill = reorder(courseSubject, -meanGrade))) + 
  ...
  labs(fill = "Subject", y = "Mean Student Grade", x = "OER") + theme_min()
```

```{r oerSemSinceImplementationGrade, caption = "Change in the mean percentage of passing students since implementation of the OER.", cache=TRUE}
SLCCpersistenceProc %>% 
  ...
  geom_col(position = "dodge") + scale_fill_manual(values = osColorPalette) + #+ 
  geom_errorbar(aes(ymin = meanGrade - semGrade, ymax = meanGrade + semGrade), width = .1, position = position_dodge(.9)) + ylim(c(0,4)) + facet_wrap(~courseSubject, scales = "free_x") + theme_min() +
...
## Simplest model
```{r}
### significant negative effect of oer (-0.10).
simpleMod1 <- SLCCpersistenceProc %>% 
  glm(pass ~ oer, data = ., family = binomial)

### Semester wins, oer becomes insignificant and smaller
simpleMod2 <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + (1|courseSubject/instructorId) + semester + oer, data = ., family = binomial)

### oer becomes insignificant and smaller
simpleMod3 <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear/semester) + (1|courseSubject/instructorId) + oer, data = ., family = binomial)

### oer becomes insignificant and smaller
simpleMod4 <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|semester/academicYear) + (1|courseSubject/instructorId) + oer, data = ., family = binomial)
plot_model(simpleMod4, type = "re")
...
```{r passRate, cache=TRUE, include=FALSE}
## Baseline model including only random effects
passMod0Base_OER <- SLCCpersistenceProc %>% 
    filter(termStartDate >= "2016-08-24") %>% 
     glmer(pass ~ semester + (1|courseSubject/instructorId) + fullTime + cumUgGpa + everPellEligibleInd, data = ., family = binomial)

passMod1_OER <- SLCCpersistenceProc %>% 
    filter(termStartDate >= "2016-08-24") %>% 
     glmer(pass ~ semester + (1|courseSubject/instructorId) + fullTime + cumUgGpa + everPellEligibleInd + oer, data = ., family = binomial)
...
SLCCpersistenceProc %>% 
    filter(termStartDate >= "2016-08-24") %>% 
    add_count(courseSubject) %>% 
    group_by(courseSubject, pass) %>% count() %>%
    #add_count(courseSubject) %>%
    summarise(passProp = sum(as.numeric(pass), na.rm = TRUE)) %>% View()
```
...
passMod0Base <- SLCCpersistenceProc %>% 
     glmer(pass ~ (1|academicYear) + semester + (1|courseSubject/instructorId), data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
```

```{r pass Additive Model 1, cache=TRUE, include=FALSE}
## Mod1: Random effects + demographics
passMod1Add <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + semester + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + everPellEligibleInd + firstGenerationInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
```

```{r pass Additive Model 2, include=FALSE}
## Mod2: Random effects + Demographics + subject/performance related factors
passMod2Add <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + semester + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + cumUgGpa + everPellEligibleInd + firstGenerationInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE, optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

```

```{r pass Additive Model 3, include = FALSE}
## Mod3: Random effects + Demographics + subject/performance related factors + treatment
passMod3Add <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + semester + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + cumUgGpa + everPellEligibleInd + oer + semSinceImplementation + firstGenerationInd, data = ., family = binomial) #, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE, optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE))
```


```{r pass Interactive model 4, cache=TRUE, include=FALSE}
...
passMod4Int <- SLCCpersistenceProc %>% 
  glmer(pass ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + gender + firstGenerationInd + fullTime + cumUgGpa + ethnicity + everPellEligibleInd + oer + semSinceImplementation + oer:ethnicity + oer:everPellEligibleInd, data = ., family = binomial)
```

## Pass model

```{r effect size, include = FALSE}
t <- SLCCpersistenceProc # %>% 
#     filter(courseSubject == "HIST")
library(effsize)
d.gpa <- cohen.d(t$cumUgGpa,t$oer, na.rm = TRUE)
d.pell <- cohen.d(as.numeric(t$everPellEligibleInd == "Y"),t$oer, na.rm = T)
d.enroll <- cohen.d(as.numeric(t$fullTime=='Full-Time'),t$oer, na.rm = T)
d.age <- cohen.d(t$age,t$oer, na.rm = TRUE)
d.firstgen <- cohen.d(as.numeric(t$firstGenerationInd == "Y"),t$oer, na.rm = T)
```

```{r LATEST OPENED MODELS}
m1 <- SLCCpersistenceProc %>%
    glmer(pass ~ semester + academicYear + courseSubject + cumUgGpa + ethnicity + everPellEligibleInd + firstGenerationInd + fullTime + (1|instructorId), family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE, optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)), data =.)
    
m2 <- SLCCpersistenceProc %>%
    glmer(pass ~ semester + academicYear + courseSubject + cumUgGpa + ethnicity + everPellEligibleInd + firstGenerationInd + fullTime + oer + (1|instructorId), family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE, optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)), data =.)

# Interactive model
m3 <- SLCCpersistenceProc %>%
    glmer(pass ~ semester + academicYear + courseSubject + cumUgGpa + ethnicity + everPellEligibleInd + firstGenerationInd + fullTime + oer + everPellEligibleInd:oer + (1|instructorId), family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE, optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)), data =.)

    
m1.m2 <- anova(m1,m2)
m1.m3 <- anova(m1,m3)

stargazer::stargazer(m4,m5, m6, type = "text")
...
```{r include=FALSE}
stargazer::stargazer(passMod0Base, passMod1Add, passMod2Add, passMod4Int, type = "text")
```


```{r pass AIC, cache=TRUE}
AIC(passMod0, passMod1Add, passMod2Add, passMod4Int) #passMod3Add, 
```
...
gradeMod0Base <- SLCCpersistenceProc %>% 
     lmer(courseGrade ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId), data = .)
```

```{r Grade Additive Omnibus Model, cache=TRUE}
## Mod1: Random effects + demographics
gradeMod1Add <- SLCCpersistenceProc %>% 
  lmer(courseGrade ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + everPellEligibleInd + firstGenerationInd, data = .)

gradeMod1AddSummary <- gradeMod1Add %>% tidy()
```

```{r grade mod 2, include=FALSE}
## Mod2: Random effects + Demographics + subject/performance related factors
gradeMod2Add <- SLCCpersistenceProc %>% 
  lmer(courseGrade ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + cumUgGpa + everPellEligibleInd + firstGenerationInd, data = .)

gradeMod2AddSummary <- gradeMod2Add %>% tidy()
```

```{r grade mod 3}
## Mod3: Random effects + Demographics + subject/performance related factors + treatment
gradeMod3Add <- SLCCpersistenceProc %>% 
  lmer(courseGrade ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + cumUgGpa + everPellEligibleInd + oer + semSinceImplementation + firstGenerationInd, data = .)

gradeMod3AddSummary <- gradeMod3Add %>% tidy()
```

```{r Grade Omnibus Model Interactive, cache=TRUE}
## Mod4: Random effects + Demographics + subject/performance related factors + treatment + interaction terms
gradeMod4Int <- SLCCpersistenceProc %>% 
  lmer(courseGrade ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + gender + firstGenerationInd + fullTime + oer + ethnicity + everPellEligibleInd + semSinceImplementation + oer:ethnicity + oer:everPellEligibleInd + oer:semSinceImplementation, data = .)

gradeMod4IntSummary <- gradeMod4Int %>% tidy()
```

#### Model comparison   

```{r course AIC}
AIC(gradeMod0Base, gradeMod1Add, gradeMod2Add,gradeMod3Add, gradeMod4Int)

stargazer::stargazer(gradeMod1Add,gradeMod2Add)
```
...
```{r prepostOERdfw, caption = "Change in mean percentage of passing students before and after introduction of OER.", cache=TRUE}
SLCCpersistenceProc %>% 
  filter(courseSubject == "HIST") %>% 
  group_by(oer, dfw) %>% 
  summarise(perc = round((sum(!is.na(dfw))/nrow(.)) * 100), groupSize = n()) %>% 
  ggplot(aes(x = oer, y = perc, fill = dfw)) + 
    geom_col(position = "dodge") + 
    geom_text(aes(label = groupSize), position = position_dodge(width = 1), vjust = -0.5) + scale_fill_manual(values = osColorPalette) +
    labs(y = "% of Passing Students", x = "OER", fill = "Not dfw (0) /DFW (1)") + theme_min()
```

```{r oerSemSinceImplementationDFW, caption = "Change in the mean percentage of dfw students since implementation of the OER.", cache=TRUE}
SLCCpersistenceProc %>% 
  add_count(oer, courseSubject,semSinceImplementation) %>% 
  group_by(oer, courseSubject, dfw, semSinceImplementation, n) %>% 
  count(dfw) %>% 
  mutate(meanDFW = round(nn/n * 100)) %>% 
  ggplot(aes(x = semSinceImplementation, y = meanDFW, fill = dfw)) + 
  geom_col() + facet_wrap(~courseSubject, scales = "free_x") + 
  labs(x = "Semester since implementation of OER", y = "Mean percentage of DFW students", fill = "DFW") + theme_min() +
  scale_fill_manual(values = osColorPalette)
```
...
dfwMod0Base <- SLCCpersistenceProc %>% 
     glmer(dfw ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId), data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))
# sjp.glmer(dfwMod0Base, facet.grid = TRUE)

dfwMod0BaseSummary <- dfwMod0Base %>% 
  tidy()
```
...
dfwMod0Base1 <- SLCCpersistenceProc %>%
     glmer(dfw ~ (1|academicYear) * (1|semester) + (courseSubject|instructorId), data = ., family = binomial, 
...
```{r dfw Additive Model 1, cache=TRUE}
## Mod1: Random effects + demographics
dfwMod1Add <- SLCCpersistenceProc %>% 
  glmer(dfw ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + everPellEligibleInd + firstGenerationInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

dfwMod1AddSummary <- dfwMod1Add %>% tidy()
## Scale/Normalize the cumulative gpa
# SLCCpersistenceProc <- SLCCpersistenceProc %>% 
#   mutate(cumUgGpaScaled = scale(cumUgGpa, center = TRUE), cumUgGpaNorm = (cumUgGpa - min(cumUgGpa, na.rm = TRUE))/(max(cumUgGpa, na.rm = TRUE) - min(cumUgGpa, na.rm = TRUE)))
```


```{r dfw Additive Model 2, cache = TRUE}
## Mod2: Random effects + Demographics + subject/performance related factors
dfwMod2Add <- SLCCpersistenceProc %>% 
  glmer(dfw ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + cumUgGpa + everPellEligibleInd + firstGenerationInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

dfwMod2AddSummary <- dfwMod2Add %>% tidy()
```


```{r dfw Additive Model 3, cache = TRUE}
## Mod3: Random effects + Demographics + subject/performance related factors + treatment
dfwMod3Add <- SLCCpersistenceProc %>% 
  glmer(dfw ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + ethnicity + gender + fullTime + cumUgGpa + everPellEligibleInd + oer + semSinceImplementation + firstGenerationInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
     optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

dfwMod3AddSummary <- dfwMod3Add %>% tidy()
```

```{r dfw Interactive Model 4, cache=TRUE, echo=FALSE, warning=FALSE, error=FALSE}
## Mod4: Random effects + Demographics + subject/performance related factors + treatment + interaction terms
dfwMod4Int <- SLCCpersistenceProc %>% 
  glmer(dfw ~ (1|academicYear) + (1|semester) + (1|courseSubject/instructorId) + age + gender + firstGenerationInd + fullTime + cumUgGpa + ethnicity + everPellEligibleInd + oer + semSinceImplementation + oer:ethnicity + oer:everPellEligibleInd, data = ., family = binomial, control = glmerControl(optimizer = "optimx", calc.derivs = FALSE,
optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE)))

dfwMod4IntSummary <- dfwMod4Int %>% tidy()
```
...
```{r echo = FALSE}
stargazer::stargazer(dfwMod0Base, dfwMod1Add, dfwMod2Add, dfwMod3Add, dfwMod4Int, type = "text")
```


```{r dfw Model Comparison, cache=TRUE}
## Compare base with additive model
dfwComp1 <- anova(dfwMod2Add, dfwMod3Add)
...
```{r withdrawalRate, include = FALSE}
# SLCCpersistenceProc %>% 
#  group_by(courseSubject, courseNumber, instructorId, academicYear) %>% 
#  summarise(`withdrawal/dropRate` = round(sum(finalGrade == "I")/n(), 4))
```

```{r withdrawalFailure, include = FALSE}
...
SLCCPersistenceSFW %>% 
  ggplot(aes(x = academicYear, y = success, color = oer, group = instructorId)) + facet_wrap(~courseSubject) + theme_min() + geom_line() + geom_point() + scale_colour_manual(values = osColorPalette) + 
...
SLCCPersistenceSFWSummary %>% 
  ggplot(aes(x = yrSem, y = meanSuccessRate, colour = oer)) + facet_wrap(~courseSubject) +  + geom_point() + geom_line(aes(x = yrSem, y = meanFailureRate), lty = 2) + labs(y = "Success", x = "Academic Year") + scale_colour_manual(values = osColorPalette) # + geom_vline(aes(xintercept = '2014 Summer'), lty = 2, lwd = 0.2)
```
...
#   glmer(pass ~ (1|studentId) + (1|instructorId) + oer + age + gender + everPellEligibleInd + fullTime + courseSubject + academicYear, data = ., family = binomial, nAGQ = 1, control = glmerControl(check.nobs.vs.nlev = 'ignore', check.nobs.vs.nRE = 'ignore'))
```
